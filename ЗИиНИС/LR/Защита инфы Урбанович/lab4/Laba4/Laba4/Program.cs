...
namespace Laba4
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            int n = k + r; //количество символов в кодовом слове
        ...
        public static int LenghtHemminga(int k) //k - количество бит 
        {
            int r = (int)(Math.Log(k, 2) + 1.99f);
            return r;
        }

        //Создание пров. матрицы r*n
        public static int[,] CheckMatrix(int k)
        {
            int r = LenghtHemminga(k); //кол-во пров. симв
            int n = r + k;
            double rDouble = r - 1;
        ...
        //Поиск синдрома - Результат умножения сообщения на выходе канала передачи (Yn)
        //или(что равнозначно) сообщения, считываемого из памяти, на проверочную матрицу(Н)
        public static int[] Sindrom(int[,] CheckMatrix, int[] mas, int k)
        {
            ...
            //Синдром – это результат проверки четности, выполняемой
            //над сообщением Yn для определения его принадлежности заданному набору кодовых слов
