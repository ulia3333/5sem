...
BOOL WINAPI HandlerRountime(DWORD eventCode)
{
	...
	HANDLE hChild1 = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE), NULL, 0, &childId_T1);*/
	switch (eventCode)
	{
...
DWORD WINAPI LifeTime(LPVOID pPrm);
...
DWORD WINAPI LifeTime(LPVOID pPrm)
{
	try {
		...
		printf("123");
		if ((lobuf = send(cC, get_message(4), 6, NULL)) == SOCKET_ERROR)
			throw  SetErrorMsgText("send:", WSAGetLastError());
...
int _tmain(int argc, char* argv[])
{
	...
	try
	{
		...
		HANDLE lifeTime = CreateThread(NULL, NULL, LifeTime,
			NULL, NULL, NULL);
